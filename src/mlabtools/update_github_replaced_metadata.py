...
for module in modules_json_data:
    ...
    if 'replaced' in module:
        new_status = module['replaced']

        yaml_content, yaml_sha = get_yaml_from_repo(module_name)
        if yaml_content:
            yaml_data = yaml.safe_load(yaml_content)
            if not 'replaced' in yaml_data:
                yaml_data['replaced'] = new_status
                updated_yaml_content = yaml.safe_dump(yaml_data)
                update_yaml_in_repo(module_name, updated_yaml_content, yaml_sha)
                print(f"Creating replaced key in metadata.yaml for {module_name}")

            elif yaml_data['replaced'] != new_status:
                yaml_data['replaced'] = new_status
                updated_yaml_content = yaml.safe_dump(yaml_data)
                update_yaml_in_repo(module_name, updated_yaml_content, yaml_sha)
